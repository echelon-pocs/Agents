...
APIs used (all free): Blockchair, Etherscan V2, Solana RPC, XRPL, CoinGecko
"""
...
def _block_from_hours_ago(hours: int, etherscan_key: str = "") -> int:
    return _latest_eth_block(etherscan_key) - (hours * 300)


def get_btc_large_transfers(min_usd: float = 2_000_000) -> List[Dict]:
    prices    = get_prices()
    btc_price = prices.get("BTC", 80000)
    min_btc   = min_usd / btc_price
    data = _get("https://api.blockchair.com/bitcoin/transactions",
                params={"limit": 50, "s": "output_total(desc)",
...
def get_eth_large_transfers(etherscan_key: str = "", min_usd: float = 1_000_000) -> List[Dict]:
    ...
    start     = _block_from_hours_ago(24, etherscan_key)
    data = _get(ESCAN, params={
        "chainid": 1, "module": "account", "action": "txlistinternal",
        "startblock": start, "endblock": 99999999, "sort": "desc",
        "apikey": etherscan_key or "YourKey",
    })
    results, seen = [], set()
    for tx in (data or {}).get("result", [])[:200]:
        if data.get("status") != "1":
            break
        val = int(tx.get("value", 0)) / 1e18
        if val >= min_eth and tx.get("hash") not in seen:
            seen.add(tx["hash"])
            results.append({
                "chain": "ETH", "hash": tx.get("hash"),
                "from": tx.get("from"), "to": tx.get("to"),
                "value_eth": val, "value_usd": val * eth_price,
                "timestamp": tx.get("timeStamp"),
            })
    return results


def get_ondo_large_transfers(etherscan_key: str = "", min_usd: float = 300_000) -> List[Dict]:
    prices     = get_prices()
    ondo_price = prices.get("ONDO", 0.45)
    start      = _block_from_hours_ago(24, etherscan_key)
    data = _get(ESCAN, params={
    ...
    for tx in (data or {}).get("result", [])[:200]:
        ...
        if value_usd >= min_usd:
            results.append({
                "chain": "ONDO", "hash": tx.get("hash"),
                "from": tx.get("from"), "to": tx.get("to"),
                "amount_ondo": amount, "value_usd": value_usd,
                "timestamp": tx.get("timeStamp"),
            })
    return results
...
def get_sol_large_transfers(min_usd: float = 1_000_000) -> List[Dict]:
    ...
    for label, address in KNOWN_WALLETS.get("SOL", {}).items():
        ...
        try:
            ...
            if r.status_code == 200:
                for sig in r.json().get("result", []):
                    results.append({"chain": "SOL", "wallet_label": label,
                                    "address": address,
...
def get_xrp_large_transfers(min_usd: float = 500_000) -> List[Dict]:
    prices    = get_prices()
    xrp_price = prices.get("XRP", 1.45)
    min_xrp   = min_usd / xrp_price
    data = _get("https://data.ripple.com/v2/transactions",
                params={"type": "Payment", "descending": "true", "limit": 50,
                        "result": "tesSUCCESS",
    ...
    for tx in (data or {}).get("transactions", []):
        ...
        if isinstance(delivered, str):
            xrp_amt = int(delivered) / 1e6
            if xrp_amt >= min_xrp:
                results.append({"chain": "XRP", "hash": tx.get("hash", ""),
                                 "from": tx.get("tx", {}).get("Account", ""),
                                 "to":   tx.get("tx", {}).get("Destination", ""),
                                 "amount_xrp": xrp_amt,
                                 "value_usd": xrp_amt * xrp_price})
    return results


def classify_transfer_direction(from_addr: str, to_addr: str) -> str:
    exchange_addrs = [a.lower() for addrs in EXCHANGE_HOT_WALLETS.get("ETH", {}).values()
                      for a in addrs]
...
def discover_profitable_eth_wallets(etherscan_key: str = "",
    ...
    """
    Scan Uniswap v3 swap events to find wallets with avg >20% profit.
    Allows losing trades — filters on average >= min_profit_pct and win rate >= 55%.
    """
    SWAP_TOPIC = "0xc42079f94a6350d7e6235f29174924f928cc2ac818eb64fed8004e115fbcca67"
    start = _block_from_hours_ago(lookback_days * 24, etherscan_key)
    data  = _get(ESCAN, params={
    ...
    for log in (data or {}).get("result", [])[:500]:
        if (data or {}).get("status") != "1":
            break
    ...
    profitable, prices = [], get_prices()
    for wallet, trades in wallet_trades.items():
        if len(trades) < 3:
            continue
        profits = []
        for i in range(0, len(trades) - 1, 2):
            bp = get_historical_price("ETH", trades[i]["date"])   or prices.get("ETH", 2500)
            sp = get_historical_price("ETH", trades[i+1]["date"]) or prices.get("ETH", 2500)
            if bp > 0:
                profits.append((sp - bp) / bp * 100)
            time.sleep(0.2)
        if not profits:
            continue
        avg      = sum(profits) / len(profits)
        win_rate = sum(1 for p in profits if p > 0) / len(profits)
        if avg >= min_profit_pct and win_rate >= 0.55:
            profitable.append({
                "address": wallet, "avg_profit_pct": round(avg, 2),
                "win_rate_pct": round(win_rate * 100, 1),
                "trade_count": len(profits), "source": "Uniswap v3 DEX scan",
                "chain": "ETH", "discovered": datetime.utcnow().strftime("%Y-%m-%d"),
            })
    return sorted(profitable, key=lambda x: x["avg_profit_pct"], reverse=True)[:10]
...
def get_all_whale_data(etherscan_key: str = "",
    ...
    btc_txs      = get_btc_large_transfers()
    eth_txs      = get_eth_large_transfers(etherscan_key)
    ondo_txs     = get_ondo_large_transfers(etherscan_key)
    xrp_txs      = get_xrp_large_transfers()
    sol_activity = get_sol_large_transfers()
    ...
    try:
        ...
        print(f"[WhaleTracker] Wallet scan failed: {e}")
        new_profitable = []
    ...
    return {
        ...
        "prices": prices,
        "large_transfers": {
            "BTC":  btc_txs[:10],  "ETH":  eth_txs[:10],
            "ONDO": ondo_txs[:10], "XRP":  xrp_txs[:10],
            "SOL":  sol_activity[:10],
        },
